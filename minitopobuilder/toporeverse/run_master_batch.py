...
def make_master(query, targetList, rmsd_cut=2.5, outfile=None, matches=None, exe=None):
    """
    """
    command = ["{}".format(exe), '--query', query, '--targetList', targetList,
               '--rmsdCut', str(rmsd_cut)]
    if outfile:
...
#SBATCH --array=0-{}\n\n"""
    ...
    for query in glob.iglob("./{}/okforms/*.pds".format(top_folder)):
        ...
        cmd = make_master(query, pds_list,
                              outfile="./{}/okforms/{}_results/_{}.master".format(top_folder, qry, qry),
                              matches="./{}/okforms/{}_results/".format(top_folder, qry),
        ...
        #    os.makedirs("./{}/okforms/{}_results/".format(top_folder, qry, trg), exist_ok=True)
        ...
        #                      exe="{}".format(master), rmsd_cut=rmsd_cut)
        #    container.append(cmd)
    with open("./{}/master.exec".format(top_folder), "w") as f:
        f.write("#!/bin/bash\n")
        f.write("SLURM_ARRAY_TASK_ID=$1\n")
        for ii, cmd in enumerate(container):
    ...
    with open("./{}/master.slurm".format(top_folder), "w") as f:
        f.write(_header.format(partits,len(container)))
        f.write("bash ./{}/master.exec ${{SLURM_ARRAY_TASK_ID}}\n".format(top_folder, qry))       
...
def submit_slurm(top_folder, partits):
    ...
    condition_files = []
    count = 0
    slurm_file = "./{}/master.slurm".format(top_folder)
    #for slurm_file in glob.iglob("./{}/master.slurm".format(top_folder)):
...
def main():
    ...
    # Make slurm scripts
    make_slurm_files(top_folder, pds, partition, master=master, rmsd_cut=rmsd_cutoff)
